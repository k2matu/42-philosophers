...
/*   Updated: 2024/07/18 22:25:02 by kmatjuhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	error_check(int argc, char **argv, t_struct *p)
{
	...
	if (p->nr_philos < 1 || p->nr_philos > 300)
		return (error_msg("Min 1 and max 300 philosopher required", FALSE));
	p->die = p_atol(argv[2]);
	p->eat = p_atol(argv[3]);
	...
	|| p->sleep < 0 || (argc == 6 && p->times_eat < 0))
		return (error_msg("Invalid input", FALSE));
	return (TRUE);
}
